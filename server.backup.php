...
require_once 'config.php';
 ...
 $responce->rows[$i]['id']=$row['id']; $id = $row['uuid'];
 if(!empty($id))$link = "<a class='text-success' href='".BASE_PATH."quiz-result.php?uuid=".$id."'>View Result</a>";
 else $link = '<span class="text-danger"> No Result </span>';
 
 $responce->rows[$i]['cell']=array($row['created'],$row['category_name'],$row['right_answer'],$row['wrong_answer'],$row['unanswered'], $link); $i++; 
 } 
